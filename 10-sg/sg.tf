module "mysql_sg" {
    source = "git::https://github.com/yeguru/terraform-aws-securitygroup.git?ref=main"
...
resource "aws_security_group_rule" "backend_vpn_alb" {
  type              = "ingress"
  from_port         = 8080
  to_port           = 8080
  protocol          = "tcp"
...
# usually we should configure frontend using private ip from vpn only but here for exception case we are using public ip without connecting the vpn.
